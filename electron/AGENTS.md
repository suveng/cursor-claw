...
- **SDK MCP 内联**：`mcp-sdk-loader.loadInlineMcpServers` 注入 stdio/command 与 HTTP/sse（经 `toHttpInlineConfig`，合并 `mcp.json` headers + `mcp-auth.json` OAuth）；`mcp-project-dir` 统一解析 Cursor projects 目录。**stdio**（`toStdioInlineConfig`）：`workspaceDir` 非空时 `cwd=workspaceDir`，并将路径型 `command`/`args` resolve 为绝对路径（segment 含 `/`、`\` 或以 `./`/`../` 开头；bare 命令名与 `--` 前缀 flag 不 resolve）；`workspaceDir` 空时不 resolve、不设 `cwd`。`Agent.create` 与 resident 模式每次 `agent.send` 均须传入 `mcpServers`（SDK inline 不持久化；Settings Path B `mcp-manager` 未改）。**注入点快照回写**：SDK 无 MCP list/status API，展示侧只能读「注入的 mcpServers 快照」——`SdkSessionAgent.lastInjectedMcpServers` 字段缓存最近一次注入。三处注入点均须回写：`buildSendOptions`（经 `appendInlineMcpToSendOptions` 返回后从 `options.mcpServers` 取值回写，不重复读盘）、`maybeRotateSessionForPressure`（提取 `injected` 变量只读盘一次，`Agent.create` 成功切换 `session.agent` 后回写；失败保留旧快照）、`launchSdkAgent`（同样提取 `injected` 一次，`Agent.create` 后在 session 字面量初始化 `lastInjectedMcpServers: injected`，首次 send 前即可读）。注入源仍 `.cursor/mcp.json`，不改 `mcp-sdk-loader`。外部模块经 `getSdkSession(sessionKey)` 取 `SdkSessionAgent` 实体读快照（interface 已 export）。
