...
- `session-dispatcher.ts`：薄 re-export 入口；调用方仍 `from './session-dispatcher'`。
- `session-dispatcher-launch.ts`：任务/工作流/`/chat new` 经 `launchSdkAgentFromHttp`（`agent-sdk-http` 统一网关）启动；**不**按引擎直 POST 独立端口。
- `session-dispatcher-chat.ts`：`/chat` 命令、`parseChatNewArgs`、工作目录校验（委托 `agent/shared/launch-request-resolve`）。
- `session-dispatcher-lifecycle.ts`：`initSessionDispatcher`、会话关闭回退、Daemon 队列、`fetchChatNames`/`fetchUserNames`。
- `session-dispatcher-runtime.ts`：四引擎运行态查询（`isSessionAgentRunning`/`stop*`/`getSessionAgentList`）；`restartCurrentSessionAgent` 仅重建/移除当前会话（SDK 原地 recreate，其它引擎冷启移除），**禁止** stopAll。
- `session-dispatcher-shared.ts`：`chatNameCache`、`extractChatId`、`isMainUser` 等无环共享工具。
- **不**扫描 IM 队列（已迁入 Daemon）；**launch 前不得**调用 `agent/shared/workspace-injector` 写盘。
- `session-mcp-status.ts`：展示层按 sessionKey 取运行时 MCP 状态的唯一入口。
- `session-mcp-disk-fallback.ts`：无活跃 session 时 Codex/OpenCode/CC 读盘 fallback（从 status 拆出，≤300 行）。
...
- **dispatch 规则**（按序短路）：(1) `getCcSession` 命中 → CC 路径三态：`getCcActiveQuery` 非空则 `await query.mcpServerStatus()`（`source:"runtime"`；抛错 `catch` 降级 snapshot，**不 crash**）→ 否则 `lastMcpServersSnapshot` 非空走 `source:"snapshot"` → 否则 `loadInlineCcMcpServers(workspaceDir)` 读盘 `source:"disk"`；(2) `getSdkSession` 命中 → `session-mcp-sdk-path.buildSdkRuntimeEntries` 合并磁盘列表与 `lastInjectedMcpServers`，标注 `rawConfig.__sdkLoadVia`（inline/settingSources/plugin），再 `fetchMcpStatusMap` probe，`source:"runtime"`；(3) 无 CC/SDK session 且 `engineType` 为 `codex`|`opencode`（或 CC 无 session 读盘）→ `session-mcp-disk-fallback.tryNoSessionDiskMcpFallback` 读盘 `source:"disk"`；(4) 其余无 session → 空态 `{ servers: [], statusMap: {}, source: "disk" }`（不抛错）。
