...
- `agent-launcher.ts`：仅 `ChatType` / `buildPrompt` / `resolveSessionChatName` 等 SDK·CC 共享符号；`buildPrompt` 有群名时首行注入 `group_name:`，无则透传 `taskMessage`（不写空占位、不注入 rules）；**无** CLI spawn。
