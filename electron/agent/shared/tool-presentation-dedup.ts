/**
 * 跨引擎工具呈现去重：相邻 running 去重与 Task 双推抑制（SDK/CC 共用）。
 */
import {
...
/** 去重所需 session 字段切片（SdkSessionAgent / CcSessionAgent 均满足） */
...
/** task 事件 status/text → 用户可见里程碑文案；taskSeq 用于无 text 时区分步骤 */
...
export function isDuplicateToolCallRunning(
  session: ToolPresentationDedupSession,
  name: string,
  args?: unknown,
): boolean {
