...
- **Presentation 时序编排（PRESENTATION_ORDERING）**：`presentationOrderingEligible(session)` = 开关开启 **且** `f41Stream && p2p`（主用户私聊 SDK）。`markProcessEventSeen(session, kind)` 在门控通过时对 **thinking / tool / task** 均置 ordering 闩（`seenProcessEvent`/`presentationDeferStream`）；**呈现抑制（daemon 里程碑降级）≠ 不参与 ordering defer**——Electron 仍 POST `presentation-event` 并置闩。含过程 Run 内 assistant delta 只累积 `streamBuffer` 不 `scheduleStreamPost`；首包未见过程事件时 `schedulePreambleRelease`（400ms 与 stream 节流对齐）短窗等待过程事件，`doFlushStreamPost` 非 final 复检 `shouldDeferAssistantPost`；过程事件 `clearStreamPostTimer`。thinking 结束 `closeThinkingIfOpen` 传 `final: true` 并 `maybeReleaseDeferredAssistant`；**task 只置闩、不单独 release**。Run 收尾 `streamRunEvents` 强制 flush。daemon 返回 `deferred: true` 时设 `presentationDeferStream`。纯对话（始终无过程事件）经 preamble 短窗后 POST，不额外延迟于现网 400ms 节流。`resetSdkRunPresentationState` / `startSdkRun` 清零编排布尔与 buffer。
- **Presentation 出站（tool/thinking/task）**：`handleSdkEvent` 中 **thinking / task 始终** `POST /api/presentation-event`；**tool_call 仅 notify 白名单** POST，silent 不产生 `presentation-event`（见下节分级门控）。`postPresentationEvent` **不因飞书抑制早退**——CardKit 抑制与里程碑降级在 daemon 处理。**tool_call 分级门控**（SSOT：`src/shared/sdk-tool-presentation-tier.ts` → `resolveSdkToolPresentationTier`）：`notify`（shell / write / strreplace / delete / task 工具名）→ `markProcessEventSeen` + `postPresentationEvent`（`running` 清 `toolPresentationOutboundIds`；非 running 时 `maybeReleaseDeferredAssistant`）；`silent`（read / glob 等未命中白名单，默认）→ **跳过** `markProcessEventSeen` 与 `postPresentationEvent`，但 `pushUiLog` `[tool]` 与 `session.lastTool` **仍全量**。**thinking**：`markProcessEventSeen` → `postPresentationEvent`。**task**：`mapTaskMilestoneText` → **`markProcessEventSeen(session, "task")` 在 `postPresentationEvent` 之前**（与 thinking/tool 对称置闩；不单独 release）→ `postPresentationEvent({ kind: "task", task_status, task_text })`。assistant 正文仍仅走 stream-text。PRESENTATION_ORDERING 仍仅 p2p。
