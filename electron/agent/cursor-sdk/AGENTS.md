...
- **Presentation 出站（tool/thinking/task）**：`handleSdkEvent` 中 `tool_call` / `thinking` / `task` → `POST /api/presentation-event`（`PresentationEvent`）。**Electron 始终 POST**（含飞书）；CardKit 抑制与里程碑降级在 daemon。**`markProcessEventSeen(session, kind)`**：仅非 `task` 且 `!feishuSuppressesProcessKind(channel, kind)` 时置 defer 闩。工具 `running` 时清该 `tool_name` 的 `toolPresentationOutboundIds` 以新建 CardKit，完成/失败回传 `outbound_message_id` 供 PATCH；assistant 正文仍仅走 stream-text。**task 分支**：独立 `case "task"` → `mapTaskMilestoneText` → `postPresentationEvent({ kind: "task", task_status, task_text })`；**不**调 `markProcessEventSeen`；daemon `handleTaskPresentationEvent` 走里程碑 `send-text`。PRESENTATION_ORDERING 仍仅 p2p。
