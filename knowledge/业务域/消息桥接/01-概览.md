...
合并卡按钮与 `/merge` 已接线 `handleMergeBatchAction`；须飞书订阅 `card.action.trigger`。CardKit 需飞书 7.20+ 且应用开通 `cardkit:card:write`（assistant 流式首选 CardKit 卡片，非 PATCH）。**launch/dispatch 调度失败**（IM 与 HTTP `POST /api/agent/dispatch`）可自动重入队并有限重试，耗尽后停试通知再 ack（不必立即手动重发）。队列策略细节见 [04](./04-消息队列与路由.md)。
