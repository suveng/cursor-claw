...
**主/次通道能力对照**（形态可不同，边界须透明）：

| 能力 | 飞书（主） | 微信（次） |
|------|------------|------------|
| 群聊过滤 | 协议 @ | 正文 @ 启发式；可 `all` |
| 进行中 | Get+CardKit | typing 4s 续期 |
| track | open_message_id | `wxc_<clientId>` |
| 合并/菜单 | 有 | 无 |
...
2026-07-12：微信次通道入队 gate、typing 续期、track 对齐（archive 20260712145946）。
2026-07-12：file-queue 子模块拆分（archive 20260712145152）。
2026-07-12：合并卡按钮与 `/merge` 斜杠双入口闭环（archive 20260712113253）。
2026-07-12：HTTP dispatch 与 IM launch 共用失败重试（archive 20260712144755）。
2026-07-12：IM launch 失败重入队与有限重试（archive 20260711232817）。
