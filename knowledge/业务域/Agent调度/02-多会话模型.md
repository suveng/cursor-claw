...
- **IM 路径三引擎**：通道 `agentResourceId` 绑定 sdk / claude-code / codex；Daemon dispatch 至 `agent-api`，由 `agent-sdk` 按资源类型路由。
...
sequenceDiagram
  ...
  participant E as sdk/cc/codex 引擎
  U->>D: IM 消息
...
**Dashboard**：`getSessionAgentList` 合并 SDK/CC/Codex；`engineType` 含 `codex`；MCP 面板 CC/SDK 取数，codex 占位。
...
- **内存**：三引擎会话 Map（含 resident、`pendingDispatch`）、`sessionAgents`（遗留）。
