...
- **会话路由持久化（T11）**：`{APP_DATA_DIR}/session-routing.json` v1 — `activeSessions` / `fallbackSessions` + per-entry `lastTouchedAt`（=该键最后一次映射触达/变更；运行期 `set*`/`clear*` 经 `mark*Touched`/`clear*Touched`，`buildSnapshot` 保真写盘、禁止全量刷新）；TTL 30d；冷启动 `setActiveSession(..., { touch: false })` 不抬升戳；6h `pruneExpiredEntries`；原子写；损坏/缺失 → 空映射 + WARN `session_routing_load_failed`。
...
- 历史同戳残差：修复前磁盘已膨胀的 `lastTouchedAt` **不**在 load 纠偏，按原戳+TTL forward 消化（可能多一段虚增空闲窗口；不可无害回溯）。
- T11-E2E：仅 kill Daemon 续聊、损坏后 launch notify 等须飞书+Electron 实机点验（契约已覆盖模块/HTTP 脏键保真与冷启动不误 mark）。
- 群聊 sessionKey 通常不含 `::` 后缀。
- MCP launch 不支持 `-dir`。

## 十、变更记录

- 2026-07-12：T11 写盘 per-entry 触达保真 + 冷启动 `touch:false`；勾销 T11-R2（20260712170533）。
- 2026-07-12：T11 `session-routing.json` 跨 Daemon 重启持久化 active/fallback 映射（archive 20260712113356）。
