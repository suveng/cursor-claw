...
- **release 串行与 Run 收尾**：`enqueueReleaseDeferredAssistantStream`+`assistantReleaseChain` 串行 release；Run 收尾仅 `flushStreamPost(true)`，无 non-final 双 POST。
...
5. **maybeReleaseDeferredAssistant**：→ `flushDeferredStreamPost` → daemon enqueue 首建。
6. **handleStreamText 飞行窗口**：占位无 outbound 时 await chain 再判 `isFirst`；非 final → `deferred: true`，防重复首建。
...
RunGuard+watchdog；400ms 节流；Run 收尾单次 `flushStreamPost(true)`。
