...
- **菜单入队**：`menu_v6` 经 `resolveMenuCommand`→`pushCommandToQueue` 写入 `.fcmd`，Main claim 后与文本斜杠**同执行路径**；权限不足/未知 key 直接 IM 回复，不入队。
...
2026-07-04：飞书私聊菜单点击映射斜杠入队，与文本斜杠并存（archive 20260704120250）。
