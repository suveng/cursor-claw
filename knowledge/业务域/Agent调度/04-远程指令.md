...
飞书/微信 `/` 指令与菜单映射：Daemon `executeSlashCommand` 即时执行；主进程指令经 `POST /api/command/execute`。合并卡四入口共用 `handleMergeBatchAction`。**仅** `dual|electron` 保留 `.fcmd` 兼容。

## 二、设计决策与取舍

- **Daemon SSOT**：`executeSlashCommand` 分 local/mcp/electron；`replyToMessage` 内联（dual poll 仍可用 `/cmd/result`）。
- **dual 去重**（仅显式 `dual`）：claim 前 `GET /commands/skip-check` + `executed-ids` 缓存（详 [Daemon 02-HTTP](../../工程平台/Daemon守护进程/02-HTTP与MCP服务.md) §五）。
- **Electron 后端**：`command-executor.executeFileCommand`；未就绪中文提示。
- **`SLASH_EXEC_MODE`**：**稳态默认 `daemon`**；显式 `dual` 双写 `.fcmd`；`electron` 回滚入队。
...
| `/list` `/clean` | 全员 | 队列列表/清空 |
| `/restart` | 全员 | 仅重建当前会话 Agent（`resolveCommandSessionKey` → `restartCurrentSessionAgent`）；不影响其他会话 |
| `/restart daemon` | 全员 | 停全部 Agent + 清队列 + 重启 Daemon |
| `/workspace` `/model` `/mcp` | 全员 | 配置 CRUD |
| `/task` `/workflow` `/chat` | 全员 | 任务/工作流/临时会话；`/workflow` 含 `resume`（paused 恢复，T10 链） |
...
sequenceDiagram
  ...
  Note over D: 仅 dual：poll claim 前 GET /commands/skip-check 去重
```
...
- **默认 `SLASH_EXEC_MODE=daemon`**；显式 `dual`/`electron` 为兼容回滚。
- README `/task trigger` vs `/task run`。

## 十、变更记录

2026-07-13：`/restart` 仅当前会话；`/restart daemon` 全量重启（20260713155326）。
2026-07-12：斜杠稳态默认 daemon（20260712144931）。
2026-07-12：`/workflow resume` 子命令（archive 20260712113344）。
