...
调度并发：`runAgentDispatchLoop` 对可推进的不同 `sessionKey` 并行 kickoff（`daemon-orchestrator-dispatch.ts`）；同会话仍串行。
...
- 关键字：`presentation_failed`、`dispatch_failed`、`dispatch_retry_*`、`agent_busy_requeue`、`dispatch_parallel`、`merge_action`。
...
- 调度：跨 session 并行；同会话 `inFlightSessions` + claim 挡 `starting`|`processing`。无 worker pool / 跨机队列。**未做**日志双写统一。
...
2026-07-12：批2 拆分；斜杠默认 daemon、dispatch retry、会话路由。
2026-06-27：IM 唯一编排、poll 移除。
