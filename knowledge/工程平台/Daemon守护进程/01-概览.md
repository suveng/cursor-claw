# Daemon 守护进程 · 概览

## 一、模块定位与边界

Daemon 是本机**唯一 IM 编排进程**：飞书 WebSocket、微信 iLink、文件队列、HTTP API、Presentation 路由与 SDK 调度转发。Agent **不再 poll-message**；经 Electron `agent-api` launch/dispatch。

## 二、整体架构

```mermaid
flowchart TB
  subgraph channels [消息通道]
    FS[飞书 WSClient]
    WX[WeChatManager]
  end
  subgraph core [Daemon 核心]
    FQ[file-queue]
    MB[MergeBatch]
    PE[Presentation 路由]
    DIS[dispatch 循环]
    HTTP[HTTP Server]
    MCP1["/mcp Agent"]
    MCP2["/mcp-admin"]
  end
  FS --> FQ
  WX --> FQ
  FQ --> DIS
  DIS -->|agent-api| EP[Electron SDK]
  EP -->|presentation-event| PE
  PE --> FS
  HTTP --> MCP1
  HTTP --> MCP2
```

## 三、核心状态机/主流程

```mermaid
sequenceDiagram
  participant IM as 飞书/微信
  participant D as Daemon
  participant Q as 文件队列
  participant E as agent-api
  IM->>D: 入站消息
  D->>Q: pushMessage
  D->>D: MergeBatch/dispatch
  D->>E: POST agent/launch
  E->>D: presentation-event
  D->>IM: CardKit 出站
  E->>D: send-text ack
  D->>Q: ackMessages
```

## 四、子模块清单

| 子模块 | 职责 | 文档 |
|---|---|---|
| HTTP 与 MCP 服务 | REST、Presentation、调度 API | [02-HTTP与MCP服务.md](./02-HTTP与MCP服务.md) |
| 进程模型与部署 | spawn、端口、agent-api 转发 | [03-进程模型与部署.md](./03-进程模型与部署.md) |

## 五、关键约束

- 启动时 `CLAW_CHANNELS_JSON` 非空（`src/daemon/daemon.ts`）。
- HTTP 仅 `127.0.0.1`；端口冲突回退随机。
- **poll-message 已移除（404）**；IM 闭环 Daemon→agent-api→Presentation。
- 代理环境变量启动时 strip。

## 六、术语表

| 术语 | 含义 |
|---|---|
| MergeBatch | 合并批次状态机 |
| PresentationEvent | SDK 展示事件 |
| agent-api-port.json | Electron SDK HTTP 端口 |
| chatKey / sessionKey | 通道路由 / Agent 会话粒度 |

## 七、依赖关系

```mermaid
flowchart LR
  Electron -->|spawn + env| Daemon
  Daemon -->|agent-api-port| Electron
  Daemon --> LarkSDK[@larksuiteoapi]
  Agent[MCP 工具] -->|send_*| HTTP
```

## 八、全局非功能与可观测

- 日志：默认 `{workspaceDir}/.cursor/daemon.log` 或 `{userData}/logs/daemon.log`；Profile 可配 `AgentResource.daemonLogPath`，经 `DAEMON_LOG_PATH` 注入 Daemon；2MB 轮转。
- `presentation_failed`、`dispatch_failed`、`dispatch_retry_scheduled`、`dispatch_retry_exhausted`、`agent_busy_requeue` 可检索。
- MCP 连接数仍参与 agentRunning 推断（任务路径）。

## 九、全局已知限制

- 斜杠指令除 `/merge` 外仍依赖 Electron claim `.fcmd`；`/merge` 在 Daemon 内闭环（`daemon-merge-command.ts`），不写 `.fcmd`。
- 合并卡按钮经 `card.action.trigger` → `feishu-card-action.ts` → `handleMergeBatchAction`；与 `/merge` 共用防抖与 `merge_action` 日志。
- admin MCP 废弃 T10。
- **IM Orchestrator launch 失败（R3）**：`releaseClaimedMessages` 重入队 + 有限重试（默认 3 次，退避 600/1200/2400 ms；busy 取 `retry_after`）；耗尽后 notify 再 `ackMessages`。成功路径仍 final/`ackOnReply` 才 ack。
- **HTTP `POST /api/agent/dispatch` 旁路**：失败仍 notify+ack、busy 仅 timer 不 release（与 launch 策略未对齐）。

## 十、文档入口

1. [03-进程模型与部署.md](./03-进程模型与部署.md) 2. [02-HTTP与MCP服务.md](./02-HTTP与MCP服务.md)

## 十一、变更记录

2026-07-12：合并卡按钮与 `/merge` 双入口（archive 20260712113253）；launch 失败重入队（archive 20260711232817）。
2026-07-02：源码路径对齐 `src/daemon/daemon.ts`（archive 20260702120154）。
2026-06-27：IM 唯一编排、Presentation、poll 移除（archive 20260627162620）。
