...
- **斜杠 SSOT**：`executeSlashCommand` + `SLASH_EXEC_MODE`（**稳态默认 `daemon`**）。
- 代理环境变量启动时 strip。
...
- 日志：`.cursor/daemon.log` 或 Profile `daemonLogPath`；2MB 轮转。
- **调度态路由恢复**：`daemonMain` load `session-routing.json`；损坏降级空映射。
...
- IM 斜杠（除 `/merge`）由 `executeSlashCommand` Daemon 内即时执行；主进程指令经 `POST /api/command/execute`。`SLASH_EXEC_MODE`：**默认 `daemon` 稳态**；显式 `dual` 双写 `.fcmd`；`electron` 回滚入队。
- `/merge` Daemon 内闭环，不入通用执行器（`daemon-merge-command.ts`）。
- 合并卡按钮 → `handleMergeBatchAction`；与 `/merge` 共用防抖。
- MCP 管理 SSOT：`POST /api/mcp` 或 IM `/mcp`；`/mcp-admin` 已移除（410）。
- **launch/dispatch 失败**：共用 `handleLaunchFailure`（release+有限重试；耗尽停试 ack）；见 [02](./02-HTTP与MCP服务.md) §三。
...
2026-07-12：斜杠稳态默认 daemon、`/mcp-admin` 移除（20260712144931）；HTTP dispatch retry（20260712144755）；会话路由/斜杠/merge（20260712113356 等）。
