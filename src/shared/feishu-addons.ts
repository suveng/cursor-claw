...
/** 飞书后台菜单 event_key → 斜杠指令映射（不含解析逻辑） */
export const FEISHU_MENU_EVENT_MAP: Record<
  string,
  { command: string; adminOnly: boolean; label: string }
> = {
  cmd_help: { command: "/help", adminOnly: false, label: "帮助" },
  cmd_status: { command: "/status", adminOnly: false, label: "查看状态" },
  cmd_reset: { command: "/reset", adminOnly: false, label: "重置会话" },
  cmd_stop: { command: "/stop", adminOnly: false, label: "停止任务" },
  cmd_workspace: { command: "/workspace", adminOnly: true, label: "工作区" },
  cmd_model: { command: "/model", adminOnly: true, label: "模型" },
  cmd_chat_new: { command: "/chat new", adminOnly: true, label: "新建会话" },
};
